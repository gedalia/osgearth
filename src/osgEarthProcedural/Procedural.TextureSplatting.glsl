...
tweakable float oe_splat_blend_start=2500.;
tweakable float oe_splat_blend_end=500.;
...
tweakable float dense_power=1.;
tweakable float lush_power=1.;
tweakable float rugged_power=1.;
tweakable float normal_power=1.;
tweakable float ao_power=1.;
tweakable float oe_depth=.02;
tweakable float oe_snow=0.;
tweakable float oe_snow_min_elev=1000.;
tweakable float oe_snow_max_elev=3500.;
tweakable float oe_splat_blend_rgbh_mix=.8;
tweakable float oe_splat_blend_normal_mix=.8;
tweakable float brightness=1.;
tweakable float contrast=1.;
...
vec2 decel(in vec2 v,in float p)
{
    ...
    {
        float d=mix(1.,oe_depth,decel(roughness,2.3));
    ...
    void resolveColumn(out Pixel pixel,int level,int x,float yvar)
    {
        ...
        float m=heightAndEffectMix(rgbh[0].a,1.-y_mix,rgbh[1].a,y_mix,rr);
    ...
    void resolveLevel(out Pixel pixel,int level,float xvar,float yvar)
    {
        ...
        float m=heightAndEffectMix(col[0].rgbh.a,1.-x_mix,col[1].rgbh.a,x_mix,rr);
    ...
    void oe_splat_Frag(inout vec4 quad)
    {
        ...
        ivec2 xy=ivec2(uv*255.);
        vec4 iquad=texelFetch(OE_LIFEMAP_TEX,xy,0);
        int special=int(iquad[SPECIAL]*255.);
        if(dense==0.&&lush==0.&&rugged==0.&&special>0)
        {
        ...
        pixel.rgbh.rgb=clamp(((pixel.rgbh.rgb-.5)*contrast+.5)*brightness,0,1);
        
        #if 1
        // perma-snow caps:
        float coldness=mapToNormalizedRange(oe_elev,oe_snow_min_elev,oe_snow_max_elev);
        float min_snow_cos_angle=1.-soften(oe_snow*coldness);
        const float snow_buf=.01;
        float b=min(min_snow_cos_angle+snow_buf,1.);
        ...
        vec3 clcolor=clamp(2.*cltexel*color,0.,1.);
        //color = mix(color, clcolor, smoothstep(0.0, 0.5, 1.0 - 0.33*(dense+lush+rugged)));
        color=mix(color,clcolor,smoothstep(0.,.5,1.-.5*(dense+lush)));
        #else
