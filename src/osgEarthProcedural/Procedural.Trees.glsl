...
void oe_GroundCover_VS_MODEL(inout vec4 geom_vertex)
{
    ...
    oe_vertex.view=oe_transform.modelview*vec4(instance[i].vertex.xyz+geom_vertex.xyz,1.);
...
void oe_GroundCover_Billboard(inout vec4 vertex_view,in uint i)
{
    ...
    vec3 halfWidthTangentVector=tangentVector*.5*width;
    ...
    float topDownAmount=rescale(d,.4,.6);
    float billboardAmount=rescale(1.-d,0.,.25);
    
    if(which<4&&billboardAmount>0.)// Front-facing billboard
    {
        vertex_view=
        which==0?vec4(vertex_view.xyz-halfWidthTangentVector,1.):
        which==1?vec4(vertex_view.xyz+halfWidthTangentVector,1.):
        which==2?vec4(vertex_view.xyz-halfWidthTangentVector+heightVector,1.):
        vec4(vertex_view.xyz+halfWidthTangentVector+heightVector,1.);
        ...
        if(billboardAmount>.1)
        {
            ...
            float blend=.25+(noise[NOISE_RANDOM_2]*.25);
    ...
    else if(which>=4&&topDownAmount>0.)// top-down billboard
    {
        ...
        float yclip=noise[NOISE_RANDOM]*.1;
        
        float k=width*.5;
        vec3 C=vertex_view.xyz+(heightVector*(.4+yclip));
        vertex_view=
        which==4?vec4(C-E*k-N*k,1.):
        which==5?vec4(C+E*k-N*k,1.):
        which==6?vec4(C-E*k+N*k,1.):
        vec4(C+E*k+N*k,1.);
    ...
    if(instance[i].modelCommand>=0)
    {
        oe_veg_transition=clamp(
            (1.+PSR_BUFFER-instance[i].pixelSizeRatio)/PSR_BUFFER,
        0,1);
    }
}
...
void oe_veg_apply_wind(inout vec4 vert_view,in float width,in float height)
{
    ...
    float xy_len=length(oe_vertex.local.xy)+oe_vertex.local.z*.2;
    ...
    float windSpeedVariation=remap(noise_moving[NOISE_CLUMPY],-.2,8.-stiffness);
    ...
    if(windSpeed>.1&&xy_comp>.6)
    {
        float buffetingSpeed=windSpeed*.2*stiffness_factor;//  xy_comp * xy_comp * xy_comp;
...
void oe_GroundCover_Model(inout vec4 vertex_view,in uint i)
{
    ...
    // apply fade from bb->model
    if(instance[i].pixelSizeRatio<1.)
    {
        oe_veg_transition=clamp(
            (instance[i].pixelSizeRatio-(1.-PSR_BUFFER))/PSR_BUFFER,
        0.,1.);
    }
...
void oe_GroundCover_FS(inout vec4 color)
{
    ...
    float a=(color.a-oe_veg_maxAlpha)/max(fwidth(color.a),.0001)+.5;
    ...
    if(cos_angle>.3){
        float snowiness=step(1.-oe_snow*coldness,cos_angle);
        color.rgb=mix(color.rgb,vec3(1),snowiness);
        oe_roughness=mix(oe_roughness,.1,snowiness);
    }
    #endif
